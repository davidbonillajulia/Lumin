...
contextBridge.exposeInMainWorld("electron", {
  ...
  savePerfSettings: (settings) =>
    ipcRenderer.invoke("save-perf-settings", settings),
  ...
  convertPptToPdf: (filePath) =>
    ipcRenderer.invoke("convert-ppt-to-pdf", filePath),
  selectOpenLuminFile: () => ipcRenderer.invoke("select-open-lumin-file"),
  selectSaveLuminFile: (defaultPath) =>
    ipcRenderer.invoke("select-save-lumin-file", defaultPath),
  selectPptFile: () => ipcRenderer.invoke("select-ppt-file"),
  writeLuminFile: (filePath, data) =>
    ipcRenderer.invoke("write-lumin-file", { filePath, data }),
  ...
  setWindowsDeviceVolume: (id, val) =>
    ipcRenderer.invoke("set-windows-device-volume", { id, val }),
  setWindowsDeviceMute: (id, mute) =>
    ipcRenderer.invoke("set-windows-device-mute", { id, mute }),
  setWindowsMute: (mute) => ipcRenderer.invoke("set-windows-mute", mute),
  setWindowsDefaultDevice: (id) =>
    ipcRenderer.invoke("set-windows-default-device", id),
  ...
  isElectron: true,
  // webUtils.getPathForFile is the correct Electron 32+ API to get the real
  // filesystem path from a File object (replaces the deprecated file.path property)
  getPathForFile: (file) => {
    try {
